...
<title>股市其他资金流动列表</title>

<meta http-equiv="pragma" content="no-cache">
<meta http-equiv="cache-control" content="no-cache">
<meta http-equiv="expires" content="0">
<meta http-equiv="keywords" content="keyword1,keyword2,keyword3">
<meta http-equiv="description" content="股市其他资金流动列表">
...
<script type="text/javascript">
			function news(){
			    	document.location.href='<%=basePath%>stockOther/add';
			}
			function del(){
				  ...
				  if(value==null){
				  	...
				  	document.location.href='<%=basePath%>stockOther/delete?id='+value;
				  }
			}
			function edit(){
			    var value = $("#table").find("input[name='cbox']:checked").val();
			    ...
			    if(value==null){
			    	...
			    	alert("该记录已记录到资产，不可修改");
			    }else{
			    	document.location.href='<%=basePath%>stockOther/edit?id='+value;
			    }
			}
			function record(){
				var value = $("#table").find("input[name='cbox']:checked").val();
			    ...
			    if(value==null){
			    	...
			    	document.location.href='<%=basePath%>stockOther/record?id='+value;
			    }
			}
			function box(cb){
...
<body bgcolor='#FFFFFF' text='#000000' leftmargin='0' topmargin='0'
	...
	<sf:form modelAttribute="form" method="GET" cssClass="listForm"
		...
		<table id="table" border="1" class="Table_N" cellspacing="0"
			...
			<tr class="Table_H">
				<td nowrap colspan="13" class="Table_H" height="40">股市其他资金流动列表：</td>
			</tr>
				...
				<td nowrap colspan="13" class="Table_H" height="40">
					<ul class="operation-button">
						...
						<li><input type="button" value="删除" onClick="del()"
							class="button"></li>
						<li><input type="button" value="记录到资产" onClick="record()"
							class="button"></li>
					</ul>
				</td>
			</tr>
			<tr class="Table_H">
				...
				<td nowrap align="center" width="30%">金额</td>
				<td nowrap align="center" width="30%">时间</td>
				<td nowrap align="center">是否已记录</td>
			</tr>

			<c:forEach items="${form.data}" var="stockOther" varStatus="i">
				<c:choose>
				...
				<tr onclick="clk(this)">
					<td align="center">
						<input type="checkbox" name="cbox"	onClick="box(this)" id="chb" value="${stockOther.id}" />
						<input type="hidden" id="${stockOther.id}" value="${stockOther.record_flag}"/>
					</td>
					<td align="center">${stockOther.name}</td>
					<td align="center">${stockOther.money}</td>
					<td align="center">
						<fmt:parseDate value="${stockOther.date}" var="date" pattern="yyyyMMdd"/>  
			            <fmt:formatDate value="${date}" type="date" dateStyle="long"/>
					</td>
					<td align="center">
						<c:if test="${stockOther.record_flag=='0'}">未记录</c:if>
						<c:if test="${stockOther.record_flag=='1'}">已记录</c:if>
					</td>
				</tr>
			</c:forEach>
		</table>
