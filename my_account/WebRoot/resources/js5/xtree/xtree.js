...
var rootpath = getJSPath("xtree.js");
...
WebFXTreeAbstractNode.prototype.checkChildren = function(bChecked) {
	...
	for(var i = 0 ;i < children.length;i++){
		if(!children[i].showInput || children[i].inputType == 'radio') continue;
...
WebFXTreeAbstractNode.prototype.setChecked = function (bChecked,checkSubTree) {
	...
	if(bChecked){
		...
		//如果存在radio父节点,并且没有选中,则选中它
			...
			//checkbox节点的子节点有一个选中的时候选择父节点
			if(this.parentNode && (!this.parentNode._checked) && this.parentNode.showInput){
				this.parentNode.setChecked(true,false);
			} 
		}catch(e){
	    	//ignore
	    }
