...
var JqueryDialog = {
	...
	Open:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight){
		JqueryDialog.init(dialogTitle, iframeSrc, iframeWidth, iframeHeight, false, false, true);
	},
	...
	init:function(dialogTitle, iframeSrc, iframeWidth, iframeHeight, isSubmitButton, isCancelButton, isDrag){
		...
		var _left = (document.body.clientWidth - (iframeWidth + JqueryDialog.cBorderSize * 2 + 5)) / 2;
		/*alert(document.body.clientWidth+","+_client_width+","+iframeWidth+","+JqueryDialog.cBorderSize+","+document.body.scrollLeft);*/
		...
		//_jd_dialog_s.css("height", iframeHeight + JqueryDialog.cBorderSize * 2 + 30 + 0 + "px");
		...
		_jd_dialog_m_b_2.append("<iframe id='jd_iframe' src='"+iframeSrc+"' scrolling='yes' frameborder='0' width='"+iframeWidth+"' height='"+iframeHeight+"' />");
		_jd_dialog_m_b_2.css("z-index", "10");
		//bottom
...
//		}
		...
		/* add by lzh @ 2012/09/12 14:47 解决IE环境下打开弹出层后，光标无法落入INPUT元素的问题 */
		$("#jd_iframe").focus();
	},
	...
	Close:function(){
		$("#jd_shadow").remove();
		$("#jd_dialog").remove();
		/* add by lzh @ 2012/09/12 14:47 解决IE环境下弹出层关闭后，光标无法落入INPUT元素的问题 */
		$(":text").each(function(){
			$(this).focus();
			return;			
		});
