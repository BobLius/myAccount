package com.miteno.common.webui.jcaptcha;
...
public class JCaptchaFilter implements Filter {
	...
	protected void initParameters(final FilterConfig fConfig) {
		if (StringUtils.isBlank(fConfig.getInitParameter(PARAM_FAILURE_URL))) {
			throw new IllegalArgumentException("CaptchaFilterȱ��failureUrl����");
		}
	...
	/**
	 * ��ApplicatonContext��ȡCaptchaServiceʵ��.
	 */
	...
	protected boolean validateCaptchaChallenge(final HttpServletRequest request) {
		try {
			...
			//����֤���ŵ�request attriubte��
			boolean validResult = captchaService.validateResponseForID(captchaID, challengeResponse);
			request.getSession().setAttribute("validCodeResult", validResult);
			return validResult;
		} catch (CaptchaServiceException e) {
			//���ͬһ�û�ͬʱ��¼���п��ܻ������쳣 
			//com.octo.captcha.service.CaptchaServiceException: Invalid ID, could not validate unexisting or already validated captcha
			logger.error(e.getMessage(), e);
			//TODO ���û�ͬʱ��¼ʱ�ȴ򿪴�ѡ�� Ϊtrue, �������Ϊfalse
			request.getSession().setAttribute("validCodeResult", false);
			return false;
		}
	}
	/**
